...
const Sidebar = ({ isOpen = true, onClose }: SidebarProps) => {
  ...
  return (
    <div className={`sidebar ${isOpen ? 'sidebar-open' : 'sidebar-closed'}`} style={sidebarStyles}>
      <div className="sidebar-inner relative flex flex-col h-full min-h-0">
        {/* Top: Close (mobile) + Logo */}
        ...
        <nav className="sidebar-nav flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-4 pb-24">
          {menuItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
        ...
        {/* Logout – sidebar ke bilkul bottom par fixed */}
        <div className="absolute bottom-0 left-0 right-0 flex-shrink-0 border-t border-gray-100 bg-white pt-4 px-4 pb-6">
          <button
            onClick={onLogout}
